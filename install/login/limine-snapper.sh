if command -v limine &>/dev/null; then
  sudo tee /etc/mkinitcpio.conf.d/hyprdots_hooks.conf <<EOF >/dev/null
...
EOF
  ...
  sudo cp $HYPRDOTS_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $HYPRDOTS_PATH/default/limine/limine.conf /boot/limine.conf

  hyprdots-pkg-add limine-snapper-sync limine-mkinitcpio-hook
