# No QR code branding needed for arch-hyprdots
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Hyprdots installation stopped!"
  show_log_tail

  gum style "This command halted with exit code $exit_code:"
  show_failed_script_or_command

  gum style "Get help by opening an issue at https://github.com/trevor-d-ndlovu/arch-hyprdots/issues"
  ...
  while true; do
    ...
    "View full log")
      if command -v less &>/dev/null; then
        less "$HYPRDOTS_INSTALL_LOG_FILE"
      else
        tail "$HYPRDOTS_INSTALL_LOG_FILE"
      fi
      ;;
