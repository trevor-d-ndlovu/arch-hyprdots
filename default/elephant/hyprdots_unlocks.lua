--
-- Dynamic Hyprdots Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the hyprdots-shipped Plymouth via
-- hyprdots-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs hyprdots-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "hyprdotsunlocks"
NamePretty = "Hyprdots Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/hyprdots/themes"
  local hyprdots_path = os.getenv("HYPRDOTS_PATH") or ""
  local default_themes_dir = hyprdots_path .. "/themes"
  local default_preview = hyprdots_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "hyprdots-launch-floating-terminal-with-presentation "
                .. shell_escape("hyprdots-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "hyprdots-launch-floating-terminal-with-presentation "
        .. shell_escape("hyprdots-plymouth-reset"),
    },
  }
