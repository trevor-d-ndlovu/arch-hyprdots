...
if hyprdots-hw-asus-expertbook-b9406 || hyprdots-hw-asus-zenbook-ux5406aa; then
  if [[ -f $EXPERTBOOK_DROP_IN ]]; then
    sudo sed -i '/xe\.enable_dpcd_backlight/d' "$EXPERTBOOK_DROP_IN"
  fi

  source "$HYPRDOTS_PATH/install/config/hardware/asus/fix-asus-ptl-display-backlight.sh"
