...
# Move user-added backgrounds from Hyprdots theme folders to user config
HYPRDOTS_DIR="$HOME/.local/share/hyprdots"
USER_BACKGROUNDS_DIR="$HOME/.config/hyprdots/backgrounds"

if [[ -d $HYPRDOTS_DIR/themes ]]; then
  cd "$HYPRDOTS_DIR"

  # Get list of git-tracked background files (relative to hyprdots dir)
...
THEMES_DIR="$HOME/.config/hyprdots/themes"
CURRENT_THEME_LINK="$HOME/.config/hyprdots/current/theme"
...
elif [[ -f $HOME/.config/hyprdots/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/hyprdots/current/theme.name")
fi

# Remove all symlinks from ~/.config/hyprdots/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  hyprdots-theme-set "$CURRENT_THEME_NAME"
else
