...
# https://github.com/trevor-d-ndlovu/arch-hyprdots/issues/1852
...
hyprdots-pkg-add xdg-terminal-exec
...
if command -v alacritty > /dev/null 2>&1; then
  cp "$HYPRDOTS_PATH/applications/Alacritty.desktop" ~/.local/share/applications/
fi
...
if [[ -f $waybar_config ]]; then
  sed -i 's|"on-click-right": "hyprdots-launch-terminal"|"on-click-right": "xdg-terminal-exec"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL --class=Wiremix -e wiremix"|"on-click": "xdg-terminal-exec --app-id=com.hyprdots.Wiremix -e wiremix"|' "$waybar_config"
  hyprdots-state set restart-waybar-required
fi

# Update hyprland window rules to use DNS-format class names
system_conf=~/.config/hypr/apps/system.conf
if [[ -f $system_conf ]]; then
  if grep -q 'class:(.*|Impala|' "$system_conf" || grep -q 'class:(.*|Wiremix|' "$system_conf" || grep -q '|Hyprdots|' "$system_conf"; then
    sed -i 's/\bImpala\b/com.hyprdots.Impala/g; s/\bWiremix\b/com.hyprdots.Wiremix/g; s/|Hyprdots|/|com.hyprdots.Hyprdots|/g' "$system_conf"
  fi
fi
