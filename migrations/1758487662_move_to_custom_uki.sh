...
if command -v limine &>/dev/null && [[ -f /etc/default/limine ]]; then
  if grep -q "^ENABLE_UKI=yes" /etc/default/limine; then
    if ! grep -q "^CUSTOM_UKI_NAME=" /etc/default/limine; then
      sudo sed -i '/^ENABLE_UKI=yes/a CUSTOM_UKI_NAME="hyprdots"' /etc/default/limine
    fi
    ...
    uki_file=$(find /boot/EFI/Linux/ -name "hyprdots*.efi" -printf "%f\n" 2>/dev/null | head -1)

    if [[ -n $uki_file ]]; then
      while IFS= read -r bootnum; do
        sudo efibootmgr -b "$bootnum" -B >/dev/null 2>&1
      done < <(efibootmgr | grep -E "^Boot[0-9]{4}\*? Hyprdots" | sed 's/^Boot\([0-9]\{4\}\).*/\1/')
