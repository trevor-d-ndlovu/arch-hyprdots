...
# Users can still opt-in to Cloudflare DNS using: hyprdots-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-hyprdots-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-hyprdots-dns.network
  ...
  echo "To use Cloudflare DNS, run: hyprdots-setup-dns Cloudflare"
fi